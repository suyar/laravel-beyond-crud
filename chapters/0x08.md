...
第四章展示了几种可能的方法将模型代码重构为独立的类，以保持你的模型类更易于管理。我们已经在我们的 Action 中测试了计算属性，但还有集合（collections）、查询生成器（query builder）和订阅者（subscriber）的测试。由于我们将功能抽象到较小的类中，所以这些测试也是非常容易的。
...
这里有一个查询生成器（query builder）测试，检查在使用 `UnitQueryBuilder::whereActive` 时是否只找到激活的数据（active units）：
