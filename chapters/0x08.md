...
在实践中，对于 `CreateInvoiceAction` 来说，这意味着我们将测试一张发票是否被正确保存和创建 —— 它的属性有被设置等 —— 但也意味着相关的发票项目被添加到这张发票上。

然而，我们不打算测试 `CreateInvoiceLineAction` 是否在内部做了它应该做的事 —— 保存一个新的发票项目及其所有属性 —— 该功能将在 `CreateInvoiceLineActionTest` 中测试，而不是这里测试。在这里，我们将假设 `CreateInvoiceLineAction` 按预期工作，我们只测试它在我们的 Action 中能够正常使用：
...
OK，这里发生了很多事情，所以让我们把它们分解一下：

首先，我们的测试有一个设置阶段，在那里我们创建一些 DTO（使用我们的工厂），并使用容器解析 Action：
...
最后，我们要做一系列断言：发票是否真正保存在数据库中，是否生成了一个数字，是否计算并保存了总价，以及是否所有的发票项目都与我们的发票相关联：
...
如果你愿意，你可以把这些断言分割成独立的测试方法，并把设置部分移到测试类的 `setUp` 函数中，我也鼓励这样做。但我也想向你展示所有的东西都结合在一起，这样你就可以看到，只需很少的设置和执行一行代码，我们就可以测试很多业务功能。

这是一个将反复出现的 Action 模式：设置（setup）、执行（execute）、断言（assert）。三个步骤，一次又一次地重复。
...
当然，我们需要一两个测试来检查 PDF 是否正确生成，但例如，在这个测试中，我们并不关心这个问题。

这便是由 Action 组成的 Action 所带来的灵活性：会有一个 `GeneratePdfAction`，它通过构造函数注入到我们的 `CreateInvoiceAction` 中：
...
现在，我们不想生成 PDF 的时候，我们都可以在例如测试类的 `setUp` 函数中调用 `MockGeneratePdfAction::setUp`，这样我们就不用再担心了!

## 测试模型（Testing models）

第四章展示了几种可能的方法将模型代码重构为独立的类，以保持你的模型类更容易管理。我们已经在我们的 Action 中测试了计算属性，但还有集合（collections）、查询生成器（query builder）和订阅者（subscriber）的测试。由于我们将功能抽象到较小的类中，所以这些测试也是非常方便的（straightforward）。
...
同样的模式出现了：设置（setup），执行（execution），和断言（assertion）。同样，这些测试是如此简单，因为我们可以使用我们的测试工厂。所以我们不必担心背后发生了什么。

这里有一个查询生成器（query builder）测试，检查在使用 `UnitQueryBuilder::whereActive` 时是否只找到有效的数据（active units）：
...
最后，测试事件监听者（subscribers）。当然，我们需要测试它是否监听了正确的事件，但也需要测试监听器（listeners）是否做了我们期望的事情。
